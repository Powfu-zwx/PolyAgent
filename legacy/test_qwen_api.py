"""Qwen API verification script using the OpenAI Python SDK."""
...
HARDCODED_API_KEY: str = "sk-dea121471f164ad3842939e1c951cc74"
...
def run_api_check() -> bool:
    """Run a single Qwen API request and print verification details."""
    try:
        from openai import (
            APIConnectionError,
            APIStatusError,
    ...
    api_key: Optional[str] = os.getenv("DASHSCOPE_API_KEY") or HARDCODED_API_KEY
    if not api_key:
        print("错误：请设置环境变量 DASHSCOPE_API_KEY，或在 HARDCODED_API_KEY 中填写。")
        return False
    ...
    try:
        ...
        print(f"网络错误：无法连接通义千问 API。详情：{exc}")
        return False
